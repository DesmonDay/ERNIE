...
class BaseReader:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.min_shot = min_shot  # 同源采样最小连续样本数
        self.max_shot = max_shot  # 同源采样最大连续样本数
        self.simplify = simplify  # 简化return_ls输出
        self.h5_output_dir = h5_output_dir  # h5 ids 文件路径
        self.add_start_token = add_start_token  # eb35 or eb4
        ...
        if self.use_agent_sys2:  # use_agent_sys2不拼接任何外挂
        ...
        with open(input_file, "r") as f:
            ...
            for line in all_lines:
                ...
                if 'ctxt_src' in data and len(data['src']) != len(data['ctxt_src']):
                    ...
                    continue
                if len(data["src"]) == 0:
                    # 部分PreSFT数据src字段为空，不适合在该样本前面再拼接样本
                    data["is_system"] = 1
                ...
                if "is_system" not in data:
                    # is_system 为 1，表示带有system，不能在该样本前拼接样本
                    data["is_system"] = 0
                if "is_q2code" not in data:
                    # is_q2code 为 1，表示是q2code数据，不能作为伪多轮的history
                    data["is_q2code"] = 0
                if "math_is_end" not in data:
                    # math_is_end 为0, 表示math中间状态情况, 不能作为伪多轮的history, 为1, 表示math最终状态情况, 为2, 表示其他
                    data["math_is_end"] = 2
                ...
                # 临时逻辑
                if self.use_agent_sys2:  # 不拼接任何外挂
                    data["disable_pseudo_multi_turn"] = 1  # 如果不以用户query为起始内容，则不采用伪多轮
                    ...
                    if self.add_sys_token or self.add_header_eos_token:
                        if data["system"] != "":
                            data["disable_pseudo_multi_turn"] = 1  # 不采用伪多轮
                            # data["is_system"] = 1 # 参与伪多轮可以做首轮
                    else:
                        if (
        ...
        for index, (example, source_to_num_opt, task_id_counter, exact_total_task_id_counter) in enumerate(
            ...
            for record in records:
                ...
                if to_append:
                    ...
                    self.batch_task_id_counter = defaultdict(int)  # 当前record没加入，则用当前record的计数器替代
                    self.batch_exact_total_task_id_counter = defaultdict(int)
    ...
    def data_generator(self):
        ...
        if phase == "train":
            ...
            for task_id, task in enumerate(tasks):
                ...
                task_sampler.set_data_status(consumed_data)  # 更新状态
                task["task_sampler"] = task_sampler  # 非同源采样器
                task["sampler"] = task_sampler.getter()  # 非同源采样器的迭代器
        ...
        def wrapper():
            ...
            for epoch_index in range(init_epoch, 100000 if phase == "train" else self.epoch):
                ...
                if phase == "train":
                    ...
                    sample_from_same_source_flags = []  # 是否同源采样标识，1：同源，0：不同源
                    ...
                    if epoch_index == init_epoch:
                        weighted_task_indices = weighted_task_indices[offset:]  # 更新状态
                        sample_from_same_source_flags = sample_from_same_source_flags[offset:]  # 更新状态
...
class KnowledgeBasedSFTReader(BaseReader):
    ...
    def _convert_example_to_record(self, example, max_seq_length, tokenizer, index):
        ...
        if self.add_sys_token or self.add_header_eos_token:
            ...
            previous_cur_len += len(system_tokens)  # 提前计算 system 长度, 后面要进行多轮裁剪
        ...
        if skip_align_flag:
            ...
            def compute_s_by_policy(skip_distance, i, turns, skip_align_s_policy="default"):
                if skip_align_s_policy == "linear":
                    # 策略1: 开始的时候少跳一点，后面多跳一点
                    skip_distance = int(skip_distance * (i + 1) / turns)  # skip_distance调小不要紧，但不能调大
                    ...
                    # 策略2: 使用beta分布实现类似效果
                    a_start, b_start = 0.5, 2.0  # 初始时偏向较小的 s
                    a_end, b_end = 2.0, 0.5  # 结束时偏向较大的 s
                    progress = i / max(turns - 1, 1)  # 确保不除以零
                    ...
                    # paper默认策略，但是会很快跳到最大距离
                    s = self.global_rng.randint(
                        1, skip_distance
                    )  # randint(a, b)取值范围是[a,b]。skip_distance=0时会报错
                return s
            ...
            # 定位X和Y的位置，确定最小单元
            assert not self.add_header_eos_token  # 第二种拼接方式的定位方法不一样，没有实现
            assert not self.knowledge_after_history  # 这个逻辑的定位方法也没有实现
            ...
            # break_turn_token_ids 会多一个: 参考724~743行
            ...
            for i in range(turns):
                # break_token: X结束，break_turn_token：Y结束
    ...
    def _pad_batch_records(self, batch_records, simplify=False):
        """
        simplify: 精简输出的return_ls，True表示需要精简.
        """
