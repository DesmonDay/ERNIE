...
def is_tensorboard_available():
    """
    判断 TensorBoard 或 TensorBoardX 是否可用。

    Args:
        无。

    Returns:
        bool: 如果 TensorBoard 或 TensorBoardX 可用，则返回 True；否则返回 False。
...
def rewrite_logs(d):
    """
    重写日志信息，将前缀为'eval_'和'test_'的日志信息分别归类到'eval/'和'test/'目录下，
    其他日志信息归类到'train/'目录下。

    Args:
        d (dict): 包含日志信息的字典，其中键为日志信息的名称，值为日志信息的内容。

    Returns:
        dict: 重新归类后的日志信息字典，其中键为重新归类的目录和日志信息名称的组合，
             值为日志信息的内容。
...
class TensorBoardCallback(TrainerCallback):
    ...
    def on_train_begin(self, args, state, control, **kwargs):
        ...
        Args:
            args (dict): 包含训练参数和配置的字典。
            state (dict): 包含训练状态的字典。
            control (dict): 包含训练控制信息的字典。
            **kwargs: 其他关键字参数。
    ...
    def on_log(self, args, state, control, logs=None, **kwargs):
        ...
        Args:
            args (object): 训练参数对象。
            state (object): 训练状态对象。
            control (object): 训练控制对象。
            logs (dict, optional): 日志信息字典。默认为 None。
            **kwargs (dict): 其他关键字参数。
        ...
        if self.tb_writer is not None:
            ...
            if data_type is not None:
                ...
                if data_type == DATATYPE_2_ID["mm"]:  # 认为一个 mini-batch 中的 data-type 是一样的
    ...
    def on_train_end(self, args, state, control, **kwargs):
        ...
        Args:
            args (argparse.Namespace): 命令行参数对象，用于保存和解析命令行参数。
            state (TrainerState): 包含当前训练状态的对象。
            control (Controller): 控制训练流程的对象。
            kwargs: 可选的参数字典。
