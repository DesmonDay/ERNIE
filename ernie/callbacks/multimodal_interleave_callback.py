...
@version: 1.0
@file: multimodal_interleave.py
@time: 2024/09/06 17:38:01
    ...
    def on_load_data_end(
        ...
        **kwargs,
        ...
        if data_type is None:
            assert hasattr(fleet.fleet, "_hcg")  # 只有 pp 激活时，才有必要在 pp 内广播数据
            ...
            for key1 in ["up_gate_proj", "down_proj"]:
                for key2 in ["weight", "bias"]:
                    ...
                    if key2 == "weight" and key1 == "down_proj":
                        model_param[key_name] = model_param[key_name] / 6  # div 6 due to 六合一
